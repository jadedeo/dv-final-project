<script>
    ...
    map.on("load", function () {
      countrySummary.forEach((country) => {
        ...
        map.on("mouseenter", layerId, (e) => {
          ...
          map.getCanvas().style.cursor = shipDataAvailable
            ? "pointer"
            : "default";
  ...
  $: if (filteredShipData.length > 0) {
    minYear = d3.min(filteredShipData, (d) =>
      d["ArrivalFormatted"]
        ? d["ArrivalFormatted"].getFullYear()
        : new Date().getFullYear()
    );
    maxYear = d3.max(filteredShipData, (d) =>
      d["ArrivalFormatted"]
        ? d["ArrivalFormatted"].getFullYear()
        : new Date().getFullYear()
    );
  ...
  $: filteredByYearShipData = filteredShipData.filter((d) => {
    return (
      d["ArrivalFormatted"] &&
      d["ArrivalFormatted"].getFullYear() <= selectedYear
    );
  });
  ...
  function selectCountry(countryName) {
    ...
    filteredShipData = shipData
      ...
      .map((d) => ({
        ...
        Passengers: d["Total Passengers"]
          ? parseInt(d["Total Passengers"], 10)
          : 10,
      }));
...
<div class="header-and-paragraphs">
  ...
  <p>
    Mauris auctor aliquam cursus. Praesent id vehicula est. Maecenas ut eros
    enim. Nulla facilisi. Lorem ipsum dolor sit amet, consectetur adipiscing
    elit. Phasellus nec euismod dui, et maximus velit. Donec nec consequat
    libero.
  </p>

  <p>
    With recent generations, many descendents of indenture have attempted to
    trace their roots. This is an extremely difficult and time consuming process
    for most, made worse by the fact that <span class="emphasis"
  ...
  <p>
    <span class="emphasis"
      ...
      information has not been compiled in any accessible or searchable form.</span
    >
    There are
    <a
      ...
      >ongoing petitions</a
    > to urge local governments to take up this important work before physical record
    books become irreversibly damaged.
  </p>
</div>
...
{#if filteredShipData.length > 0}
    ...
    <di id="dot-settings">
      <div id="dot-legend">
        <div class="dot-legend-label row">
          <div
            class="ship"
            style=" background-color:unset; {`border: 1px solid ${selectedCountryColor}`}"
          ></div>
  ...
  <div id="units">
    {#each filteredByYearShipData as ship, index}
      <div
        class="ship"
        style="border: {ship['Total Passengers']
          ? ''
          : `1px solid ${selectedCountryColor}`}; background-color: {ship[
          'Total Passengers'
        ]
          ? selectedCountryColor
          : 'unset'}; opacity: {getOpacity(ship['Total Passengers'])}"
  ...
  {#if selectedShip !== null}
    <div id="voyage-details" transition:slide={{ duration: 500 }}>
      ...
      <p>
        Arrived in {filteredByYearShipData[selectedShip]["Country"]} on
        <span class="emphasis"
          >{filteredByYearShipData[selectedShip][
            "ArrivalFormatted"
        ...
        {#if filteredByYearShipData[selectedShip]["Total Passengers"]}
          <span
            >carrying <span class="emphasis"
              >{filteredByYearShipData[selectedShip]["Total Passengers"]} passengers</span
            ></span
          >
          ...
          {#if filteredByYearShipData[selectedShip]["Total Passengers"]}
            <span
              >with <span class="emphasis"
                >registration numbers {filteredByYearShipData[selectedShip][
